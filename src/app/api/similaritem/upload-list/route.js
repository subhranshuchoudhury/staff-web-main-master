...
export const POST = async (req) => {
    try {
        ...
        const itemName = bodyData.itemName.toUpperCase().trim();
        const similarItem = bodyData.similarItem.toUpperCase().trim();

        const preCheckItem = await SimilarItem.findOne({ 'similarList.itemName': similarItem }).select('itemName');


        if (preCheckItem) {
            if (preCheckItem.itemName === itemName) {
                return NextResponse.json({ message: "Already saved, No modification required" }, { status: 200 });
            } else {
                const removeItem = await SimilarItem.findOneAndUpdate({
                    itemName: preCheckItem.itemName,
                }, {
                    $pull: {
                ...
                if (removeItem) {
                    ...
                    if (findNewItem) {
                        ...
                        if (result) {
                            ...
                            return NextResponse.json({ message: `We couldn't update the similar item in (${itemName})` }, { status: 400 });
                        }
                    } else {
                    ...
                    return NextResponse.json({ message: `The item is already mapped with another item (${preCheckItem.itemName}), and we couldn't remove it` }, { status: 400 });
                }
            }
