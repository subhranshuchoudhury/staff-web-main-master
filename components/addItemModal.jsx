...
import unitypes from "../src/app/DB/Purchase/unitypes";
import gstAmount from "../src/app/DB/Purchase/gstamount";
import Image from "next/image";
import CustomOption from "../src/app/Dropdown/CustomOption";
import CustomMenuList from "../src/app/Dropdown/CustomMenuList";
import xlsx from "json-as-xlsx";
import { uploadItem } from "../src/app/AppScript/script";
...
export default function AddItemModal(props) {
    ...
    const handlePopUp = (value) => {
        props.handleAddItemPopUp(value)
    }
    ...
    const uploadItemList = async (d) => {
        ...
        window.addItemMessageModal.showModal();
        ...
        try {
            ...
            props.setDetailsOfAddItemModal(d);
            ...
            if (response === "200") {
                ...
                setModalMessage({
                    message: "Check your internet connection before try again.",
                    title: "Failed ❌",
                    btn: "Ok",
                });
    ...
    const checkForUnsaved = () => {
        if (localStorage.getItem("US_ADDED_ITEMS")) {
            ...
            if (localStorage.getItem("PURCHASE_NOT_DOWNLOAD_DATA")) {
                ...
                setModalMessage({
                    message: `Unsaved items found ${US_INV || ""
    ...
    const pushContent = () => {
        ...
        if (isEmpty) {
            setModalMessage({
                message: "Kindly fill all the details before uploading the item.",
                title: "Empty 🤷‍♂️",
                btn: "Ok",
            });
            ...
            .then((response) => {
                if (response.status === 200) {
                    ...
                    if (isApp !== null || isApp !== undefined) {
                        ...
                        // window.open("/history/purchase/share/latest?download=1")
                    }
                } else {
                    setModalMessage({
    ...
    return (
        <>
            <Toaster />
            <dialog id="addItemMessageModal" className="modal">
            ...
            <dialog id="addItemModal" suppressHydrationWarning={true} onClose={() => handlePopUp(false)} className="modal">
                <div className="modal-box">
                    <form method="dialog">
                        {/* if there is a button in form, it will close the modal */}
                        <button className="btn btn-sm btn-circle btn-ghost absolute right-2 top-2 scale-150">✕</button>
                    </form>
                    ...
                    <div className="glass bg-blue-800 flex justify-between p-5 rounded-lg">
                        <button
                            onClick={() => {
